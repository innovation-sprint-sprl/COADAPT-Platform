...
namespace UserManagement.WebAPI.Controllers {
	[Route("1.0/psychologicalReports")]
	[ApiController]
	[Authorize(Policy = "Everyone")]
	public class PsychologicalReportController : ControllerBase {
		private readonly ILoggerManager _logger;
		private readonly IRepositoryWrapper _coadaptService;

		public PsychologicalReportController(ILoggerManager logger, IRepositoryWrapper repository) {
			_logger = logger;
		...
		[HttpGet]
		[ProducesResponseType(typeof(PsychologicalReport), StatusCodes.Status200OK)]
		...
		public async Task<IActionResult> GetAllPersonalInformation([FromQuery(Name = "fromDate")] DateTime fromDate) {
			...
			if (role == Role.ParticipantRole) {
				...
				return fromDate == DateTime.MinValue ?
					Ok(await _coadaptService.PsychologicalReport.GetPsychologicalReportsByParticipantIdAsync(requestingUser.Id)) :
					Ok(await _coadaptService.PsychologicalReport.GetPsychologicalReportsAfterDateByParticipantIdAsync(fromDate, requestingUser.Id));
			}
			if (role == Role.AdministratorRole) {
				return fromDate == DateTime.MinValue ? 
					Ok(await _coadaptService.PsychologicalReport.GetPsychologicalReportsAsync()) : 
					Ok(await _coadaptService.PsychologicalReport.GetPsychologicalReportsAfterDateAsync(fromDate));
			}
			var participants = await _coadaptService.Participant.GetAllParticipantsAsync();
			var filteredPersonalInformations = new List<PsychologicalReport>();
			foreach (var participant in participants) {
				if (role == Role.SubAdministratorRole) {
					...
					if (fromDate == DateTime.MinValue) {
						filteredPersonalInformations.AddRange(await _coadaptService.PsychologicalReport.GetPsychologicalReportsByParticipantIdAsync(participant.Id));
					} else {
						filteredPersonalInformations.AddRange(await _coadaptService.PsychologicalReport.GetPsychologicalReportsAfterDateByParticipantIdAsync(fromDate, participant.Id));
					}
		...
		public async Task<IActionResult> GetAllPersonalInformationOfParticipant(string code, [FromQuery(Name = "fromDate")] DateTime fromDate) {
			...
			return fromDate == DateTime.MinValue ?
				Ok(await _coadaptService.PsychologicalReport.GetPsychologicalReportsByParticipantIdAsync(participant.Id)) :
				Ok(await _coadaptService.PsychologicalReport.GetPsychologicalReportsAfterDateByParticipantIdAsync(fromDate, participant.Id));
		}
		...
		public async Task<IActionResult> GetPersonalInformationById(int id) {
			var personalInformation = await _coadaptService.PsychologicalReport.GetPsychologicalReportByIdAsync(id);
			if (personalInformation.IsEmptyObject()) {
				_logger.LogWarn($"GetPersonalInformationById: PsychologicalReport with ID {id} not found!");
				return NotFound("PsychologicalReport with requested ID does not exist");
			}
		...
		public async Task<IActionResult> DeletePersonalInformation(int id) {
			var personalInformation = await _coadaptService.PsychologicalReport.GetPsychologicalReportByIdAsync(id);
			if (personalInformation.IsEmptyObject()) {
				_logger.LogDebug($"DeletePsychologicalReport: PsychologicalReport with id {id} not found.");
				return NotFound("PsychologicalReport with requested id does not exist");
			}
			string userId = HttpContext.User.Claims.Single(x => x.Type == "id").Value;
			string role = HttpContext.User.Claims.Single(x => x.Type == "http://schemas.microsoft.com/ws/2008/06/identity/claims/role").Value;
			...
			_coadaptService.PsychologicalReport.DeletePsychologicalReport(personalInformation);
			await _coadaptService.SaveAsync();
			return NoContent();
		}
