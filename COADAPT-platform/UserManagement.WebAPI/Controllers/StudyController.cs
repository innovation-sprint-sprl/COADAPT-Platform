...
namespace UserManagement.WebAPI.Controllers {
	...
	public class StudyController : ControllerBase {
		...
		/// </remarks>
		[HttpGet]
		[ProducesResponseType(typeof(StudyListResponse), StatusCodes.Status200OK)]
		...
		public async Task<IActionResult> GetAllStudies() {
			...
			return Ok(await _coadaptService.Study.StudiesBySupervisor(supervisor.Id));
		}
		...
		public async Task<IActionResult> GetStudyById(int id) {
			...
			study.Groups = (ICollection<Group>)await _coadaptService.Group.GroupsByStudy(id);
			return Ok(study);
		}

		/// <summary>
		///	Retrieve the study with given short name and parent organization ID
		/// </summary>
		/// <remarks>
		/// Remarks:
		/// - Only administrators, sub-administrators and supervisors can retrieve a study
		...
		[HttpGet("{shortName}/{organizationId}", Name = "StudyByShortNameAndOrganizationId")]
		[ProducesResponseType(typeof(Study), StatusCodes.Status200OK)]
		[ProducesResponseType(StatusCodes.Status404NotFound)]
		[ProducesResponseType(StatusCodes.Status500InternalServerError)]
		public async Task<IActionResult> GetStudyByShortNameAndOrganizationId(string shortName, int organizationId) {
			var study = await _coadaptService.Study.GetStudyOfOrganizationByShortnameAsync(shortName, organizationId);
			if (study.IsEmptyObject()) {
				_logger.LogError($"GetStudyById: Study with short name {shortName} not found in organization with ID {organizationId}.");
				return NotFound("Study with requested short name and parent organization ID does not exist");
			}
			string userId = HttpContext.User.Claims.Single(x => x.Type == "id").Value;
			string role = HttpContext.User.Claims.Single(x => x.Type == "http://schemas.microsoft.com/ws/2008/06/identity/claims/role").Value;
			if (role == Role.SubAdministratorRole) {
			...
			study.Supervisor.User = await _userManager.FindByIdAsync(study.Supervisor.UserId);
			study.Organization = await _coadaptService.Organization.GetOrganizationByIdAsync(study.OrganizationId);
			study.Sites = (ICollection<Site>)await _coadaptService.Site.SitesByStudy(study.Id);
			study.Groups = (ICollection<Group>)await _coadaptService.Group.GroupsByStudy(study.Id);
			return Ok(study);
		}

		/// <summary>
		///	Retrieve the study with given path of short names [organization short name].[study short name]
		/// </summary>
		/// <remarks>
		/// Remarks:
		/// - Only administrators, sub-administrators and supervisors can retrieve a study
		...
		[HttpGet("names/{names}", Name = "StudyByShortNamesPath")]
		[ProducesResponseType(typeof(Study), StatusCodes.Status200OK)]
		[ProducesResponseType(StatusCodes.Status404NotFound)]
		[ProducesResponseType(StatusCodes.Status500InternalServerError)]
		public async Task<IActionResult> GetStudyByShortNamesPath(string names) {
			var nameComponents = names.Split(".");
			if (nameComponents.Length != 2) {
				_logger.LogWarn($"GetStudyByShortNamesPath: Expected two short names separated py a period: [organization short name].[study short name]");
				return BadRequest("Expected two short names separated py a period: [organization short name].[study short name]");
			}
			var organization = await _coadaptService.Organization.GetOrganizationByShortnameAsync(nameComponents[0]);
			if (organization.IsEmptyObject()) {
				_logger.LogWarn($"GetStudyByShortNamesPath: Organization with short name {nameComponents[0]} does not exist!");
			...
			if (study.IsEmptyObject()) {
				_logger.LogError($"GetStudyByShortNamesPath: Study with short name {nameComponents[1]} not found in organization with short name {nameComponents[0]}.");
				return NotFound("Study with requested short name and parent organization does not exist");
			}
			string userId = HttpContext.User.Claims.Single(x => x.Type == "id").Value;
			string role = HttpContext.User.Claims.Single(x => x.Type == "http://schemas.microsoft.com/ws/2008/06/identity/claims/role").Value;
			if (role == Role.SubAdministratorRole) {
				...
				if (organizationOfSubAdmin.IsEmptyObject()) {
					_logger.LogWarn($"GetStudyByShortNamesPath: Currently logged in sub-administrator is not assigned to any organization!");
					return BadRequest("Currently logged in sub-administrator is not assigned to any organization");
				}
				if (organization.Id != organizationOfSubAdmin.Id) {
					_logger.LogWarn($"GetStudyByShortNamesPath: Requested study is not part of the organization of currently logged in sub-administrator!");
					return BadRequest("Requested study is not part of the organization of currently logged in sub-administrator");
				}
			}
			if (role == Role.SupervisorRole) {
				var supervisor = await _coadaptService.Supervisor.GetSupervisorByUserIdAsync(userId);
				if (study.SupervisorId != supervisor.Id) {
					_logger.LogWarn($"GetStudyByShortNamesPath: Requested study is not supervised by currently logged in supervisor!");
					return BadRequest("Requested study is not supervised by currently logged in supervisor");
				}
			}
			study.Supervisor = await _coadaptService.Supervisor.GetSupervisorByIdAsync(study.SupervisorId);
		...
		/// - Cannot delete study that has sites or groups; reassign or delete them first
		/// </remarks>
		/// <param name="id"></param>
		[Authorize(Policy = "SubAdministrator")]
		[HttpDelete("{id}")]
		...
		public async Task<IActionResult> DeleteStudy(int id) {
			...
			if (await _coadaptService.Site.CountSitesByStudy(id) > 0) {
				_logger.LogError($"DeleteStudy: Cannot delete study that has sites.");
				return BadRequest("Cannot delete study that has sites");
			}
			if (await _coadaptService.Group.CountGroupsByStudy(id) > 0) {
				_logger.LogError($"DeleteStudy: Cannot delete study that has groups.");
				return BadRequest("Cannot delete study that has groups");
			}
