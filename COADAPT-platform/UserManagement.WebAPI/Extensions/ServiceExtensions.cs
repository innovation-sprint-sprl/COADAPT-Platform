...
namespace UserManagement.WebAPI.Extensions {
	...
	public static class ServiceExtensions {
		...
		/*
		 * Python settings configuration method.
		 */
		public static void ConfigureDecryptionSettings(this IServiceCollection services, IConfigurationSection configuration) {
			services.Configure<DecryptionSettings>(configuration);
		}
		...
		public static void ConfigureSwagger(this IServiceCollection services, string apiVersion) {
			services.AddSwaggerGen(c => {
				c.SwaggerDoc(apiVersion, new Info { Version = apiVersion, Title = "COADAPT User Management API", License = new License { Name = "Apache License 2.0", Url = "https://github.com/innovation-sprint-sprl/COADAPT-Platform/blob/master/LICENSE" } });

				c.AddSecurityDefinition("Bearer", new ApiKeyScheme { Description = "JWT Authorization header using the Bearer scheme. Example: \"Bearer {token}\"", Name = "Authorization", In = "header", Type = "apiKey" });
