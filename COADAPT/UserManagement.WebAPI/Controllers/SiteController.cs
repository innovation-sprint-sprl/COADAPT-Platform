...
namespace UserManagement.WebAPI.Controllers {
	...
	public class SiteController : ControllerBase {
		...
		public async Task<IActionResult> GetAllSites() {
			...
			if (role == Role.SubAdministratorRole) {
				...
				foreach (var study in studiesOfOrganization) {
					sites.AddRange(await _coadaptService.Site.SitesByStudy(study.Id));
				}
				return Ok(sites);
			}
		...
		/// <summary>
		///	Retrieve a specific site with given ID
		/// </summary>
		/// <remarks>
		/// Remarks:
		...
		public async Task<IActionResult> GetSiteById(int id) {
			...
			string role = HttpContext.User.Claims.Single(x => x.Type == "http://schemas.microsoft.com/ws/2008/06/identity/claims/role").Value;
			if (role == Role.AdministratorRole) {
				site.Study = await _coadaptService.Study.GetStudyByIdAsync(site.StudyId);
				return Ok(site);
			}
		...
		public async Task<IActionResult> UpdateSite(int id, [FromBody]SiteRequest siteRequest) {
			...
			if (role == Role.SubAdministratorRole) {
				...
				if (originalStudy.OrganizationId != organization.Id) {
					_logger.LogWarn($"UpdateSite: Study of updeted site does not belong to the organization of the currently logged in sub-administrator!");
					return BadRequest("Study of updeted site does not belong to the organization of the currently logged in sub-administrator");
				}
			} else if (role == Role.SupervisorRole) {
				supervisor = await _coadaptService.Supervisor.GetSupervisorByUserIdAsync(userId);
				if (originalStudy.SupervisorId != supervisor.Id) {
					_logger.LogWarn($"UpdateSite: Study of updeted site does not belong to the currently logged in supervisor!");
					return BadRequest("Study of updeted site does not belong to the currently logged in supervisor");
				}
			}
		...
		public async Task<IActionResult> DeleteSite(int id) {
			...
			if (((ICollection<StudyParticipant>)await _coadaptService.StudyParticipant.StudyParticipantsBySite(id)).Count > 0) {
